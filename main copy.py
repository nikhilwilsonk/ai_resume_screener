...
# Load NLP Model and SentenceTransformer
...
class Resume(Base):
    ...
    # This field can store the most recent match percentage for convenience.
    match_percentage = Column(Float)
    
    # Relationship to job matches
...
async def upload_resume(
    ...
    try:
        ...
        # If the resume exists and overwrite is True, update it.
        if existing_resume:
        ...
        # Always insert a new job match record for this upload
        job_match = JobMatch(
...
async def get_uploads(
    ...
    db = SessionLocal()
    try:
        # Join resumes and job_matches so that each upload entry is available.
